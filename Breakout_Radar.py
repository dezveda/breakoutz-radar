...
# ==============================================================================
# CAPA DE DATOS: ARQUITECTURA RADAR CON BINANCE FEED & SCORING ENGINE
# ==============================================================================
class AsyncExchange(QThread):
    ...
    async def analyze_symbol(self, symbol: str, sem: asyncio.Semaphore):
        async with sem:
            try:
                df_fast = await self.feed.fetch_historical_klines(
                    symbol=symbol,
                    interval=self.cfg.TIMEFRAME_FAST,
                    limit=self.cfg.LOOKBACK_PERIODS
                )
                ...
                first_open = float(df_fast['open'].iloc[0])
                price_change_pct = ((last_price - first_open) / (first_open + 1e-9)) * 100.0
                vol_sum = float(df_fast['volume'].sum())
                vol_m = (vol_sum * last_price) / 1_000_000.0

                flags = eval_res.get('flags', [])
                direction = eval_res.get('direction', 'NEUTRAL')
                details_str = " ".join(flags) if flags else direction
    ...
    async def run_loop(self):
        ...
        while self.running:
            try:
                ...
                if not symbols:
                    self.status_signal.emit("⚠️ NO PAIRS FETCHED, RETRYING...")
                ...
                self.status_signal.emit(f"🔬 EVALUATING {len(active_symbols)} PAIRS...")

                sem = asyncio.Semaphore(self.cfg.MAX_CONCURRENT_REQUESTS)
                tasks = [self.analyze_symbol(sym, sem) for sym in active_symbols[:60]]
                ...
                self.status_signal.emit(f"⚠️ SCAN ERROR: {str(e)}")
                await asyncio.sleep(5)
